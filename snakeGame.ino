...
void runSnake()
{
  ...
  if (nbPlayer > MAXSNAKEPLAYER)
    nbPlayer = MAXSNAKEPLAYER;
 ...
 */
    ...
    for (j = 0; j < nbPlayer; j++)
    {
      ...
      if (xs[j][0] < 0)
      {
        xs[j][0] = FIELD_WIDTH - 1;
      }
      else if (xs[j][0] >= FIELD_WIDTH)
      {
        xs[j][0] = 0;
      }
      else if (ys[j][0] < 0)
      {
        ys[j][0] = FIELD_HEIGHT - 1;
      }
      else if (ys[j][0] >= FIELD_HEIGHT)
      {
        ys[j][0] = 0;
      }
      //      }
    ...
    for (j = 0; j < nbPlayer; j++)
    {
      ...
      {
        if (isDead[j])
          setTablePixel(xs[j][i], ys[j][i], RED);
        else
        {
          if (i == 0)
            setTablePixel(xs[j][i], ys[j][i], snakecolhead[j]);
          else
  ...
  if (nbPlayer > 2)
    printNumber(score[1], 0, 5, snakecol[2]);
  if (nbPlayer > 3)
    printNumber(score[1], 8, 5, snakecol[3]);
